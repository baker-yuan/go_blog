...
func MenuRegister(group *gin.RouterGroup, repo repo.MenuRepo) {
	menu := &MenuController{
		menuService: service.NewMenuService(repo),
	}
	group.GET("/admin/user/menus", menu.ListUserMenus)
}
...
func (m MenuController) ListUserMenus(ginCtx *gin.Context) {
	var (
