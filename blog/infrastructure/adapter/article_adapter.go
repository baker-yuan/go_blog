...
func (a *ArticleAdapter) articleSearch(search map[string]interface{}, currentPage uint32, pageSize uint32) ([]*po.ArticlePO, uint32, error) {
	var (
		DB       = db.GetMysqlDb()
		articles = make([]*po.ArticlePO, 0)
		total    int64
	)
	var (
		current uint32
	...
	tx = DB.Table("tb_article").Offset(int(current)).Limit(int(pageSize))
...
func (a *ArticleAdapter) ListArchives(currentPage uint32, pageSize uint32) ([]*dto.ArchiveDTO, uint32, error) {
	var (
	...
	var (
		dbArticles []*po.ArticlePO
		search     map[string]interface{}
	)
	search = map[string]interface{}{
		"is_delete": constant.LogicDeleteFalse,
		"status":    entity.ArticleStatusPublic,
	}
...
func (a *ArticleAdapter) ListArticleBacks(condition cqe.ConditionVO, currentPage uint32, pageSize uint32) ([]*dto.ArticleBackDTO, uint32, error) {
	var (
		DB       = db.GetMysqlDb()
...
func FindTagByArticleIds(articleIds []uint32) map[uint32][]dto.TagDTO {
	...
	DB.Raw("select a.article_id as articleId, a.tag_id as tagId, b.tag_name as tagName from tb_article_tag a join tb_tag b on a.tag_id = b.id where a.article_id in ?", articleIds).Scan(&raws)
...
func FindCategoryByCategoryIds(categoryIds []uint32) map[uint32]string {
	var (
		DB       = db.GetMysqlDb()
		category = make([]po.CategoryPO, 0)
		result   = make(map[uint32]string, 0)
	)
	DB.Raw("select * from tb_category where id in ?", categoryIds).
		Scan(&category)
...
func (a *ArticleAdapter) FindById(articleID uint32) (*entity.Article, error) {
	var (
		DB        = db.GetMysqlDb()
		dbArticle = po.ArticlePO{}
		tx        *gorm.DB
	)
	var (
		article = entity.Article{}
	)
	tx = DB.Raw("select * from tb_article where id in =", articleID).
		Scan(&dbArticle)
	if tx.Error != nil {
		return nil, tx.Error
	}
...
func (a *ArticleAdapter) Save(article *entity.Article) error {
	return nil
}
