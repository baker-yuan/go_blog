...
// ArticleUI 文章服务
type ArticleUI interface {
}
type ArticleUIImpl struct {
	app app.ArticleApp
...
func (a ArticleUIImpl) ListArticleBacks(ginCtx *gin.Context) {
	var (
		currentPage uint32
		pageSize    uint32
	...
	if err := params.BindValidParam(ginCtx); err != nil {
		middleware.ResponseErrorWithCode(ginCtx, middleware.FAIL)
		return
	}
	// 搜索
...
func (a ArticleUIImpl) SaveOrUpdateArticle(ginCtx *gin.Context) {

}

func (a ArticleUIImpl) UpdateArticleTop(ginCtx *gin.Context) {
	var (
...
func (a ArticleUIImpl) ListArchives(ginCtx *gin.Context) {
	var (
		currentPage uint32
		pageSize    uint32
	)
