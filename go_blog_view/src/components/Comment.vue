    ...
    <div v-if="count > 0 && reFresh">
      ...
      >
        ...
        <div class="comment-meta">
          ...
          >
            ...
            <div class="reply-meta">
              ...
              <div class="comment-user">
                ...
                <span class="blogger-tag" v-if="reply.uid === 1">博主</span>
              </div>
              <!-- 信息 -->
              <div class="comment-info">
                <!-- 发表时间 -->
              ...
              <p class="comment-content">
                <!-- 回复用户名 -->
                <template v-if="reply.replyUid !== item.uid">
          ...
          <!-- 回复框 -->
          <Reply :module-code="moduleCode" :object-id="objectId" ref="reply" @reloadReply="reloadReply" />
        </div>
      </div>
      <!-- 加载按钮 -->
...
export default {
  ...
  methods: {
    ...
    replyComment(index, item) {
      ...
      //传值给回复框
      this.$refs.reply[index].replyUid = item.uid;
    ...
    listComments() {
      ...
      this.axios
        ...
        .then(({ data }) => {
          this.commentList.push(...data.data.rows);
        });
    },
    /**
     * 添加评论
    ...
    insertComment() {
      ...
      this.content = "";
      this.axios.post("/ics/comment/add", comment).then(({ data }) => {
        if (data && data.status === 200) {
          //查询最新评论
          this.$emit("reloadComment");
          this.$toast({ type: "success", message: "评论成功～～～" });
        } else {
          this.$toast({ type: "error", message: data.desc });
        }
      });
    },
