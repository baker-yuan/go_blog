      ...
      <div class="article-info-container">
        ...
        <div class="article-info">
          ...
          <div class="third-line">
            ...
            <!-- 评论量 -->
            <span>
              <i class="iconfont iconpinglunzu1" />评论数: {{ article.commentCount }}
            </span>
          </div>
        </div>
      </div>
    ...
    <v-row class="article-container">
      ...
      <v-col md="9" cols="12">
        <v-card class="article-wrapper">
          ...
          <div class="article-reward">
            ...
            <a class="reward-btn" v-if="blogInfo.websiteConfig.rewardFlag === true && article.appreciation === true">
              <!-- 打赏按钮 -->
          ...
          <div class="pagination-post">
            ...
            >
              <router-link :to="'/articles/' + article.lastArticle.id">
                <img
                  class="post-cover"
                  :src="article.lastArticle.coverImage"
                />
                <div class="post-info">
            ...
            >
              <router-link :to="'/articles/' + article.nextArticle.id">
                <img
                  class="post-cover"
                  :src="article.nextArticle.coverImage"
                />
            ...
            <div class="recommend-list">
              ...
              >
                <router-link :to="'/articles/' + item.id">
                  <img class="recommend-cover" :src="item.coverImage" />
          ...
          <!-- 评论 -->
          <comment
            v-if="blogInfo.websiteConfig.commentFlag === true && article.commentEnabled"
      ...
      <v-col md="3" cols="12" class="d-md-block d-none">
        <div style="position: sticky;top: 20px;">
          <!-- 文章目录 -->
          <v-card class="right-container directory-container">
            <div class="right-title">
              <i class="iconfont iconhanbao" style="font-size:16.8px" />
              <span style="margin-left:10px">目录</span>
            </div>
            <div class="directory" id="toc" />
          </v-card>
          <!-- 最新文章 -->
...
export default {
  ...
  created() {
    ...
    window.handCodeOpen = this.handCodeOpen;// 解决字符串模板@click无效的问题
  },
  mounted() {
  },
  destroyed() {
  ...
  methods: {
    handCodeOpen(el) {
      console.log('handCodeOpen', el)
      // todo 待优化
      el.parentNode.parentNode.parentNode.classList.remove('set-code-hide');
      let parentNode = el.parentNode.parentNode;
      for (let childNodesKey of parentNode.childNodes) {
        parentNode.removeChild(childNodesKey);
      }
      // let setCodeHide = this.findFirstParentByClassName(el, 'set-code-hide')
      // let parentNode = this.findFirstParentByClassName(el, 'hide-preCode-box-father');
      // if (setCodeHide === undefined || setCodeHide === null) {
      //   return
      // }
      // console.log('setCodeHide', setCodeHide)
      // console.log('parentNode', parentNode)
      // setCodeHide.remove('set-code-hide')
      // // console.log('parentNode', parentNode)
      // for (let childNodesKey of parentNode.childNodes) {
      //   parentNode.removeChild(childNodesKey);
      // }
    ...
    findFirstParentByClassName(el, className) {
    ...
    getArticle() {
      ...
      //查询文章
      this.axios.post("/blog/article/detail", {
        'id': articleId
      }).then(({ data }) => {
        if (!printErr(data)) {
          return
    ...
    like() {
      ...
      if (this.article.likeStatus) {
        url = "/ics/owner/cancelObjectPraise"
      } else {
        url = "/ics/owner/objectPraise"
      }
    ...
    markdownToHtml(article) {
      ...
      const md = new MarkdownIt({
        ...
        highlight: function(str, lang) {
          ...
          let setCodeHide = '';
          if (linesLength > 40) {
            setCodeHide = 'set-code-hide';
            hiddenCode = `<div class="hide-preCode-box-father"><div class="hide-preCode-box"><span onClick="handCodeOpen(this)" class="hide-preCode-bt"><span class="look-more-preCode contentImg-no-view"></span></div></div>`;
          }
          // todo vue无法渲染，临时改成text
          if (!lang || !hljs.getLanguage(lang)) {
            // console.log("无法渲染的语言->" + lang);
            lang = "text";
          }
          // highlight.js 高亮代码
          ...
          return`<pre class="hljs ${setCodeHide}">${hiddenCode}<code>${html}</code>${linesNum}</pre>
          <textarea style="position: absolute;top: -9999px;left: -9999px;z-index: -9999;" id="copy${codeIndex}">${str.replace(/<\/textarea>/g, "</textarea>")}</textarea>`;
  ...
  computed: {
    ...
    articleCover() {
      return (
        "background: url(" + this.article.coverImage + ") center center / cover no-repeat"
      );
    },
    isLike() {
      return this.article.likeStatus ? "like-btn-active" : "like-btn";
    },
    isFull() {
      return function(id) {
